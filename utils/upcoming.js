module.exports = async (client) => {
    ...
    const Embed = new Discord.MessageEmbed()
        ...
        .setFooter(`ORLA - Last Updated: ${moment.unix(moment().unix()).tz(client.config.timezone).format('Do MMMM, h:mma z')}`, client.config.logo)
    
    for (const event of events) {
        if (client.hosts[event.host] !== undefined) {
            const icon = client.emojis.cache.get(client.hosts[event.host].emoji)
            const time = moment.unix(event.ttime).tz(client.config.timezone).format('dddd Do MMMM, h:mma z')

            let rtime = ''
            if (moment.unix(event.rtime).tz(client.config.timezone).format('h:mm') !== moment.unix(event.ttime).tz(client.config.timezone).format('h:mm')) {
                rtimeFormat = (moment.unix(event.rtime).tz(client.config.timezone).format('dddd') === moment.unix(event.ttime).tz(client.config.timezone).format('dddd')) ? 'h:mma z' : 'dddd Do MMMM, h:mma z'
                rtime = `\nRego Closes:** ${moment.unix(event.rtime).tz(client.config.timezone).format(rtimeFormat)} **`
            }
